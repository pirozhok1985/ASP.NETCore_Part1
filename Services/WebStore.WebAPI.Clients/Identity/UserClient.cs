...
public class UserClient : BaseClient, IUserClient
{
    ...
    public IQueryable<User> Users => Get<IEnumerable<User>>($"{Address}/all")!.AsQueryable();
    ...
    {
        await PostAsync($"{Address}/AddLogin", new LoginDto { User = user, LoginInfo = login }, cancel).ConfigureAwait(false);
    }
    ...
    public async Task SetLockoutEndDateAsync(User user, DateTimeOffset? endDate, CancellationToken cancel)
    {
        var response = await PostAsync(
                ...
                new LockoutDto { User = user, LockoutEndTime = endDate },
                cancel)
           .ConfigureAwait(false);
